...
$vitals = $db->query($sql);
  ...
  $sql = "
    ...
    FROM assign_doctors ad
    JOIN rooms r ON ad.room_id = r.id
    JOIN users s ON ad.doctor_id = s.id
    WHERE ad.status = 1
";
            ...
            <div class="card-body" style="padding: 24px;">
                ...
                <div class="patient-search-section">
                  <!--   <div class="search-container" style="width: 100%; margin-bottom: 24px;">
                    ...
                    <div class="selected-patient-info" id="selectedPatientInfo" style="">
                        <div class="patient-card">
                            ...
                            <div class="patient-details">
                                ...
                                <div class="patient-meta">
                                    <span class="patient-id">Hospital Number: <?=$appointment['hospital_num']?></span>
                                    <span class="patient-age"><?=getAge($appointment['dob'])?> years</span>
                                    <span class="patient-gender"><?=$appointment['gender']?></span>
                                </div>
                                <div class="patient-status">
                                    <span class="status-badge status-critical"><?=get('room_name','rooms',$appointment['room_id'])?></span>
                                    <span class="arrival-time">Arrived: <?= date('d D M Y h:i:s', strtotime($appointment['date_appointed'])) ?>
</span>
                                </div>
                            </div>
                ...
                <div class="vitals-form-section">
                    ...
                    <form id="vitalsForm" method="POST" action="process_vitals.php">
                        <!-- Vital Signs Grid -->
                            ...
                            <?php foreach($vitals as $v){
                               ...
                               $vital_value =  !(empty($values[$vital_id])) ? $values[$vital_id] : ""   
                                ...
                                <div class="input-with-unit">
                                    <input type="text"  name="vital_<?=$vital_id?>_<?=$appointment_id?>" class="form-control" id="inputPassword3" value="<?=$vital_value?>"   <?=($appointment['appointment_status']!=0) ? 'readonly' : '' ?> required>
                        ...
                        <div class="form-group" style="margin-top: 24px;">
                            ...
                            <textarea name="nurse_notes" id="vitalsNotes" rows="3" placeholder="Enter any additional observations, symptoms, or concerns..." <?=($appointment['appointment_status']!=0) ? 'readonly' : '' ?> ><?=!empty($appointment['nurse_notes']) ? $appointment['nurse_notes'] : ''?></textarea>
                        </div>
                        ...
                        <div class="form-actions" style="margin-top: 32px; display: flex; gap: 12px; justify-content: flex-end;">
                            ...
                            <button type="submit" class="btn-primary" id="saveVitalsBtn">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 8px;">
             ...
             <!--    <div class="room-filter" style="margin-bottom: 24px;">
                ...
                <div class="rooms-grid">
                    ...
                    <?php foreach($rooms as $room){
                           $num_patients = getNumberOfPatient($room['room_id']);
                        ?>
                         <div class="room-card available" data-type="general">
                        <div class="room-header">
                            <span class="room-number"><?=$room['room_name']?></span>
                            <span class="room-type"><?=$num_patients?></span>
                        </div>
                        <div class="room-details">
                            <div class="room-info">
                              <!--   <span class="room-bed">Bed A</span> -->
                                <span class="room-status available"><?=$room['doctor_name']?></span>
                            </div>
                        ...
                        <button class="assign-btn" onclick="assignToRoom(<?=$room['room_id']?>,<?=$appointment['appointment_id']?>)">
...
function assignToRoom(room,appointment_id) {
       swal("Are you sure?", "This Patient Will be assigned to this room", "warning", {
        buttons: ["Cancel", "Yes, assign"]
    }).then((ok) => {
        if (ok) window.location.href = 'assign_room.php?room=' + room+'&appointment='+appointment_id;
    }); 
}
